...
st.set_page_config(page_title="雙人資產與曝險計算器", layout="centered", page_icon="💰")
...
if quick_mode:
    ...
    total_stock_profit, total_roi = 0.0, 0.0
else:
    st.subheader("💵 現金資產配置")
    ...
    with col_in1: input_symbol = st.text_input("第一步：請輸入台股股號 (如: 2330)")
    with col_in2: check_btn = st.button("🔍 查詢股票資訊")
    ...
    if current_list:
        ...
        del_list = [i for i, r in df_display.iterrows() if st.checkbox(f"刪除 {r['股票名稱']} ({r['股號']})", key=f"del_{i}")]
    ...
    else: stock_cost_total, stock_mv_total, stock_exposure_total = 0.0, 0.0, 0.0
...
if not quick_mode: st.metric("🏆 投資組合總帳面損益", f"{total_stock_profit:+.2f} 萬元", f"總整體投報率 {total_roi:+.2f}%")
...
if 1.35 <= exposure_ratio <= 1.50: st.success(f"✅ 狀態安全：當前總曝險為 **{exposure_ratio:.2f}**，符合 1.35 - 1.50 黃金區間。")
    ...
    st.info(f"💡 建議：獲利了結約 **{suggested_reduce/2:.1f} 萬元** 轉為現金。")
else: st.warning(f"ℹ️ 提示：目前總曝險低於 1.35。")
...
if not user_history.empty:
    ...
    with tab2: st.plotly_chart(px.line(user_history, x="日期", y="實質曝險比率走勢", markers=True), use_container_width=True)
